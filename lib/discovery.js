...
function skillsUnder(root) {
  ...
  for (const name of readdirSync(base)) {
    ...
    if (existsSync(skillMd)) {
      out.push({ id: `skill:${name}`, kind: 'skill', path: skillMd, dir: join(base, name), repo_root: detectRepo(join(base, name)) });
    }
  }
  return out;
}
...
export function discoverSkills(projectDir, home = homedir()) {
  ...
  return all.filter(t => !t.dir.includes(join('.claude', 'plugins', 'cache')));
}
